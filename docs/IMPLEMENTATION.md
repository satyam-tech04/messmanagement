...
| **Store pre-submission check**         | ✅ API on www.mealadda.in, in-app legal links, INTERNET declared — 6 manual blockers left, see RELEASE-MOBILE.md                                                                                                                                       |
| **Account deletion (D-32)**            | ✅ in-app request, immediate lock-out, admin erase queue — 18 live checks pass (`npm run verify:account-deletion`)                                                                                                                                     |
| **Force-update check (D-33)**          | ✅ `/api/app-version` + blocking screen; `MIN_APP_BUILD` env. Could not be retrofitted after release                                                                                                                                                   |
| **Push notifications (D-34)**          | ✅ Firebase `mealadda-49d4f` configured; both platforms build with config verified inside the artifacts. **Not yet delivered to a real device**; Vercel env var still to set. Plan reminders have no scheduler                                         |
| **Server-driven config + ads (D-35)**  | ✅ `/superuser/app-config` + `/api/app-config`; adaptive banners on all 4 student screens behind per-screen toggles. **Google sample AdMob ids** until the account is restored — `npm run verify:release-ads` blocks a release                         |
| **Android release pipeline**           | ✅ `npm run release:android` — preflight, verify + flutter analyze/test, upload-key signature check, filed to ~/Desktop/MealAdda-releases. Upload key created 2026-09-22. Play Console app created and set up; closed test (12 testers × 14 days) next |
...
**Aurora Depth revamp, 15 Sep 2026** — both apps repainted; landing page at `/`; operator
persona chooser with audited student impersonation; admin-only web sign-in built behind
`WEB_SIGNIN_APP_ONLY` (off). Proven against the live DB with throwaway accounts in
`demo-hostel` only. **Owner to-dos:** replace the pricing and about-us copy in
`src/app/(marketing)/content.ts` with real figures; flip the switch (RUNBOOK §6a) once
Campus Crave is on the app.
...
`GET /api/me`. **Bundle id decided: `com.mealadda.app`, display name MealAdda.**
...
The app lives in **`mobile/`**, in this repo. Bundle id `com.mealadda.app` on both platforms.
