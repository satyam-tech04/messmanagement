...
>
> **Status: shipped 2026-08-30** (commit `4eb46b1`, migration 013). The text below is the
...
> | ---------- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
> | throughout | "Grace Period"                                                                                                     | **`subscription_pauses`**, and "pause" everywhere in code. `student_status.GRACE` has existed since migration 001 and means the _opposite_ — unpaid dues, still allowed to eat. Sharing the name is how a paused student gets fed or a dues-grace student gets refused at the counter. The admin UI still says "Pause plan". |
> | §7 vs §8   | §7: the admin types the end date and the system must not overwrite it. §8: the subscription gains the paused days. | These contradict. **D-18**: the system computes `anchor + paused days`, pre-fills it, the admin may override, and _both_ values are stored so a divergence is visible.                                                                                                                                                       |
> | §22        | "must avoid accumulating extensions" — no mechanism given                                                          | **`end_date_before_pause`** is frozen when the pause is first created. Every recalculation is `end_date_before_pause + days actually paused`, so the fourth edit gives the same answer as the first. This is the load-bearing column of the whole feature.                                                                   |
> | §13        | Scheduled → Active → Completed, as if something flips them                                                         | **Derived from the dates.** There is no cron here beyond the headcount snapshot, so a stored flag would need a job that does not exist. AC9's "automatic resume" is true _because_ nothing runs.                                                                                                                             |
> | §5.1, AC2  | a pause may not start today                                                                                        | **D-15 overrides this: today is allowed**, past is not. An admin told at 9am that a student went home this morning must be able to act now.                                                                                                                                                                                  |
> | §3         | excludes only _cancelled_ subscriptions                                                                            | Also excludes **expired** ones. The spec omits it; there is nothing left to pause.                                                                                                                                                                                                                                           |
> | §14        | "do not create overlapping Grace Periods"                                                                          | Enforced by an **`EXCLUDE USING gist`** constraint on a half-open daterange, not an application check — only a constraint holds when two admins act at once. Non-overlapping _repeats_ stay legal: a student may go home twice.                                                                                              |
> | §1, §18    | "three user roles"                                                                                                 | **Four.** `SUPER_ADMIN` is the platform support account and is treated as an admin throughout. `role === "ADMIN"` would lock it out.                                                                                                                                                                                         |
> | §25        | "There is currently no audit functionality in the application"                                                     | **False.** `public.audit_log` has existed since migration 001 with 20+ call sites. §25 is read as forbidding a _new_ audit system; the existing one is written to.                                                                                                                                                           |
> | §5 dates   | no timezone                                                                                                        | Every date goes through `src/core/time`, and the counter check is against the meal's `service_date`, not `now()` — a 00:30 dinner belongs to the previous day.                                                                                                                                                               |
>
> Live proof: `npm run verify:pause`. Full reasoning: [TRACKER.md](./TRACKER.md) findings F5–F12.
